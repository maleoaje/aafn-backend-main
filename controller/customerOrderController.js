...
const addOrder = async (req, res) => {
  try {
    console.log("addOrder called with user:", req.user);
    const orderData = {
      ...req.body,
    };

    // Add user field only if user is authenticated
    ...
    // Update product quantities before responding
    try {
      await handleProductQuantity(order.cart);
    } catch (quantityErr) {
      console.error("Error updating product quantity:", quantityErr);
...
const sendEmailInvoiceToCustomer = async (req, res) => {
  try {
    ...
    const body = {
      from: req.body.company_info?.from_email || "sales@allamericanfoodsnetwork.com",
      to: user.email,
